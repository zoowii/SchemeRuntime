...
SFormItem* s_func_define(SchemeRuntime* runtime, SFormItem** params,
		size_t params_count) {
	if (params_count < 2) {
	...
	if (firstItem->type == ID && params_count == 2) // define variable
		...
		if (funcNameItem->type != ID) {
			throw new Exception(
					"define func need accepts a identifier as funcName");
		}
		...
		for (size_t i = 1; i < firstForm->size(); i++) {
			...
			if (paramItem->type != ID) {
				throw new Exception(
						"when define func, params should be identifier");
			}
			// TODO: 检测参数是否有重复
...
SFormItem* s_func_lambda(SchemeRuntime* runtime, SFormItem** params,
		size_t params_count) {
	if (params_count < 2) {
...
SFormItem* s_func_display(SchemeRuntime* runtime, SFormItem** params,
		size_t params_count) {
	string str;
...
SFormItem* s_func_number_add(SchemeRuntime* runtime, SFormItem** params,
		size_t params_count) {
	int sum = 0;
	for (size_t i = 0; i < params_count; i++) {
		SFormItem* param = params[i]->eval(runtime);
...
SFormItem* s_func_number_sub(SchemeRuntime* runtime, SFormItem** params,
		size_t params_count) {
	int sum = 0;
	for (size_t i = 0; i < params_count; i++) {
		SFormItem* param = params[i]->eval(runtime);
...
SFormItem* s_func_number_less(SchemeRuntime* runtime, SFormItem** params,
		size_t params_count) {
	if (params_count < 2) {
...
SFormItem* s_func_number_less_or_equal(SchemeRuntime* runtime,
		SFormItem** params, size_t params_count) {
	if (params_count < 2) {
		throw new Exception("< at least accepts 2 params");
	}
	bool result = true;
...
SFormItem* s_func_number_bigger(SchemeRuntime* runtime, SFormItem** params,
		size_t params_count) {
	if (params_count < 2) {
		throw new Exception("> at least accepts 2 params");
	}
...
SFormItem* s_func_number_bigger_or_equal(SchemeRuntime* runtime,
		SFormItem** params, size_t params_count) {
	if (params_count < 2) {
		throw new Exception("> at least accepts 2 params");
	}
...
SFormItem* s_func_cond(SchemeRuntime* runtime, SFormItem** params,
		size_t params_count) {
	...
	for (size_t i = 0; i < params_count; i++) {
		...
		if (param->size() < 2) {
			throw new Exception(
					"cond only accepts params which are forms and at least have 2 items");
		}
		SFormItem *condItem = (*param)[0]->eval(runtime);
		if (condItem->type != BOOLEAN) {
			throw new Exception(
					"cond's params must be forms whose first item is of type boolean");
		}
...
SFormItem* s_func_if(SchemeRuntime* runtime, SFormItem** params,
		size_t params_count) {
	if (params_count < 2) {
...
SFormItem* s_func_equal(SchemeRuntime* runtime, SFormItem** params,
		size_t params_count) {
	if (params_count < 2) {
		throw new Exception("= at least accepts 2 params");
	}
...
SFormItem::SFormItem(SchemeRuntime* runtime, string funcName,
		NativeFuncType funcType, bool isNative) :
		type(ORIGINAL_FUNC) {
	switch (funcType) {
...
SFormItem::SFormItem(SchemeRuntime* runtime, TokenType type) :
		type(type) {
	runtime->itemsPool->add(this);
}

SFormItem::SFormItem(SchemeRuntime* runtime, char* strVal, TokenType type) :
		type(type) {
	if (type == NUM) {
...
SFormItem::SFormItem(SchemeRuntime* runtime, const char* strVal, TokenType type) :
		type(type) {
	if (type == NUM) {
		this->value.numVal = atoi(strVal);
	} else {
...
SFormItem::SFormItem(SchemeRuntime* runtime, bool boolVal) :
		type(BOOLEAN) {
	this->value.boolVal = boolVal;
	runtime->itemsPool->add(this);
}

SFormItem::SFormItem(SchemeRuntime* runtime, list<string> *funcParamNames,
		SFormList *funcBody, list<FunctorNamePairValuePair> *funcBoundParams) :
		type(SS_FUNCTOR) {
	...
	if (funcBoundParams != NULL) {
		...
		this->value.functor.funcBoundParams =
				new list<FunctorNamePairValuePair>();
	}

	runtime->itemsPool->add(this);
}

SFormItem::SFormItem(SchemeRuntime* runtime, list<SFormItem> *list) :
		type(LIST) {
...
SFormItem::~SFormItem(void) {
	if (this->type == STRING) {
		...
		list<FunctorNamePairValuePair>::const_iterator it =
				this->value.functor.funcBoundParams->begin();
...
string SFormItem::toString() const {
	...
	if (this->type == LIST) {
		...
		if (this->value.functor.funcBoundParams->size() == 0) {
			...
			ss << "<Function(" << this->value.functor.funcBoundParams->size()
					<< ")(" << this->value.functor.funcParamNames->size()
					<< ")>";
		}
...
SFormItem* SFormItem::eval(SchemeRuntime *runtime) const {
	if (this->type == ID) {
		...
		if (value == NULL) {
			throw new Exception(
					string("unknown identifier: ") + *this->value.strVal);
		} else {
			return value;
		}
	} else {
...
SFormItem* SFormItem::apply(SchemeRuntime* runtime, SFormItem** params,
	...
	if (this->type == ORIGINAL_FUNC) {
		...
		bool useCurEnv = runtime->getEnv()->isLastStmt
				&& runtime->getEnv()->curProcess == this; // judge if it's tail recursive call, if true, use the curEnv but clear it
		if (!useCurEnv) {
			runtime->downEnv();
						runtime->getEnv()->curProcess = this;
		}
		if (params_count > needed_params_count) {
			...
			list<FunctorNamePairValuePair>::const_iterator it =
					this->value.functor.funcBoundParams->begin();
			while (it != this->value.functor.funcBoundParams->end()) {
				runtime->pushToCurEnv(string(*(it->name)),
						it->value->eval(runtime));
				it++;
			}
			size_t i = 0;
			list<string>::const_iterator it1 =
					this->value.functor.funcParamNames->begin();
			...
			// currying
			list<FunctorNamePairValuePair> *funcBoundParams = new list<
			...
			result = new SFormItem(runtime, funcParamName,
					this->value.functor.funcBody, funcBoundParams);
		}
		if (!useCurEnv) {
			runtime->upEnv();
		}
